            ...
            <div class="card">
              ...
              <?php 
                ...
                if($_POST){//this line solve the problem of unknown searchName error in first landing page after login
                  ...
                  $stmt = $connection->prepare("SELECT * FROM categories WHERE name LIKE '%$searchKey%' ORDER BY id DESC");
                  $stmt->execute();
                  $result = $stmt->fetchAll();
                  $totalPages = ceil(count($result)/$numberOfRecords);
                }else{
                  $totalPages = ceil(count($rawResult)/$numberOfRecords);
                  $stmt = $connection->prepare("SELECT * FROM categories ORDER BY id DESC LIMIT $offset,$numberOfRecords ");
                  $stmt->execute();
                  $result = $stmt->fetchAll();
                }
                ...
                <table class="table table-bordered">
                  ...
                  <tr>
                      ...
                      <td>
                        <div class="btn-group">
                          <div class="container">
                            <a href="category_edit.php?id=<?php echo $value['id']?>" type="button" class="btn btn-primary" >edit</a>
                          </div>
                          <div class="container">
                            <a href="category_delete.php?id=<?php echo $value['id']?>" type="button" class="btn btn-warning" onclick="return confirm('Are you sure you want to delete')">Delete</a> 
                          </div>
                        </div>
                      </td>
                    </tr>
